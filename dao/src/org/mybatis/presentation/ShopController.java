...
public class ShopController {
	...
	public ModelAndView list(Shop shop) throws Exception {
		...
		/* 서비스 객체 호출 */
		List<Shop> listShop = this.shopService.list(shop);
	...
	public ModelAndView view(@PathVariable (value = "shopNo") int shopNo) throws Exception {
		ModelAndView modelAndView = new ModelAndView("/shop/view");
		/* 서비스 객체 호출 */
		modelAndView.addObject("shop", this.shopService.view(shopNo));
	...
	public ModelAndView editForm(@PathVariable(value = "shopNo") int shopNo) throws Exception {
	...
	/* 가게 수정  (완료) */
	@RequestMapping(value = "/edit/{shopNo}", method = RequestMethod.POST)
	...
	public ModelAndView remove(@PathVariable (value = "shopNo") int shopNo) throws Exception {
		RedirectView redirectView = new RedirectView("/shop/list");
		redirectView.setExposeModelAttributes(false);
		/* 서비스 객체 호출 */
